...
#       either:
#         - ':percent' (e.g. '10% off each item')
#         - ':dollar' (e.g. 'Save $5 on each item')
#         - ':fixed_price' (e.g. '5 for $50')
#     - 'discount_amount' is the percentage/dollar discount to
#       apply (per item) unless :fixed_price is used. In that case the
#       discount_amount will actually be the total value of those items.
...
PRODUCT_DISCOUNT_TIERS = [
  {
    ...
    product_selectors: ["promo-panties-5for25"],
    tiers: [
      {
        quantity: 3,
        discount_type: :fixed_price,
        discount_amount: 20,
        discount_message: '3 for $20',
      },
      {
        quantity: 5,
        discount_type: :fixed_price,
        discount_amount: 30,
        discount_message: '5 for $30',
      },
    ],
  },
]
...
class DiscountApplicator
  def initialize(discount_type, discount_amount, discount_message, quantity)
  ...
  def run(cart)
    @campaigns.each do |campaign|
      ...
      discount_applicator = DiscountApplicator.new(
        ...
        applicable_tier[:quantity]
      )
      
      applicable_items.each do |line_item|
        line_quantity = line_item.quantity
  
        if line_quantity > applicable_tier[:quantity]
          take_quantity = applicable_tier[:quantity]
